...
//			where T : Component;
	...
	//		ManageAnimations(shakeAnim,null);
		...
		public void UpdateStrengthBarAnim(Agent ba){
			magicBar.maxValue = ba.maxStrength;
			magicText.text = ba.strength + "/" + ba.maxStrength;


			if (firstSetStrengthBar) {
				magicBar.value = ba.strength;
			} else {
				magicBar.DOValue (ba.strength, 0.5f);
			}
