...
namespace WordJourney
{
	...
	public class DoubleAttack : ActiveSkill {
		...
		protected override void ExcuteSkillLogic(BattleAgentController self, BattleAgentController enemy){
			...
			while (attackTime < 2) {
				...
				if (enemy.agent.reflectScaler > 0) {
					...
					self.PlayHurtTextAnim (hurtStr, TintTextType.None,0.1f * attackTime);
				}
				...
				if (actualPhysicalDamage > 0) {
					...
					enemy.PlayHurtTextAnim (physicalHurtStr, tintTextType, 0.1f * attackTime);
				}
				...
				if (actualMagicalDamage > 0) {
					...
					enemy.PlayHurtTextAnim (magicalHurtStr, TintTextType.None, 0.2f + 0.1f * attackTime);
				...
				if (self.agent.healthAbsorbScalser > 0) {
					...
					self.PlayGainTextAnim (gainStr,0.1f * attackTime);
				}
